...
class kaoqinqiandao(BaseModel):
    __doc__ = u'''kaoqinqiandao'''
    __tablename__ = 'kaoqinqiandao'
    ...
    qiandaoshiduan=models.CharField ( max_length=255, null=True, unique=False, verbose_name='签到时段' )
    qiandaoliucheng=models.CharField ( max_length=255, null=True, unique=False, verbose_name='签到流程' )
    gengxinshijian=models.DateField   (  null=True, unique=False, verbose_name='更新时间' )
    '''
    biaoti=VARCHAR
    tupian=Text
    qiandaoshiduan=VARCHAR
    qiandaoliucheng=VARCHAR
    gengxinshijian=Date
    '''
    class Meta:
        db_table = 'kaoqinqiandao'
        verbose_name = verbose_name_plural = '考勤签到'
class qiandaojilu(BaseModel):
    ...
    __isAdmin__='否'#表属性isAdmin=”是”,刷出来的用户表也是管理员，即page和list可以查看所有人的考试记录(同时应用于其他表)
    addtime = models.DateTimeField(auto_now_add=False, verbose_name=u'创建时间')
    qiandaobianhao=models.CharField ( max_length=255, null=True,unique=True, verbose_name='签到编号' )
    biaoti=models.CharField ( max_length=255, null=True, unique=False, verbose_name='标题' )
    zhaopian=models.TextField   (  null=True, unique=False, verbose_name='照片' )
    qiandaoshijian=models.DateField   (  null=True, unique=False, verbose_name='签到时间' )
